...
object Main extends TaskApp {
  ...
  /**
   * Server.stream[Task] gives us a Resource[Task, Server[Task]]. Resource, a monad type that is a description of how to acquire manage and cleanly release something with a lifecycle. Similar to try/finally but composable and referentially transparent. Allows tracking resource lifetime at type level (Function accepting Resource[]IO, A. parameter will acquire an release internally, Function accepting A reuses the live value) Allows guaranteed finalization.
